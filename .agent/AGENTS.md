...
- `model_router.py "<subtask>" --runner codex --policy auto:balanced` —
  read-only model recommendation for agent delegation. It never changes the
  active session, permissions, or user settings.
- `personal_memory.py "<intent>"` — read-only static profile, current work,
  and relevant reviewed memory for an agent. Keep profile writes behind the
