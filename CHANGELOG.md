...
### Added
- A full-page knowledge graph workspace that collapses the filters, note list,
  and detail pane into a responsive canvas. It preserves the current camera,
  selected memory, and 2D/3D choice, with Escape and visible controls to exit.
- An interactive 3D knowledge graph with perspective depth, provider clusters,
  depth-aware links, node selection, drag-to-orbit, pinch/button zoom, reset,
  and a persistent 2D/3D switch. The 2D map remains available without changing
  graph data, search, filters, or provenance.
- Native conversation references for Claude Code, Codex, OpenCode, and Cursor.
  Type an `@` tool mention in the composer, search detected local chats, and
  attach up to five frozen references to the next message.
- Contextual plain-`@` suggestions across every saved workspace, with filters
  for coding tool, model, primary agent, subagent, automation, and review agent.
  Claude Code, Codex, Cursor, and OpenCode subagent sessions retain their public
  model, role, parent, and project metadata when available.
- Prompt-aware relevance ranking and background index prewarming, so the picker
  can recommend related conversations while the user writes and cached filter
  changes remain immediate.
- Read-only OpenCode SQLite and Cursor JSONL discovery, plus opt-in imports for
  both sources in the shared knowledge graph.
- Per-run conversation provenance, kept separate from automatic memory
  retrieval so reviewers can see exactly what the user attached.
- A local stdio MCP bridge with conversation search/read tools, recent-chat
  resources, shared-graph search, and reversible setup for all four tools.
- Global `agentic-stack-context` skills for Claude Code, Codex, OpenCode, and
  Cursor, so literal `@Claude`, `@Codex`, `@OpenCode`, and `@Cursor` references
  work even when a host does not expose MCP resources in its native `@` menu.
- Read-only MCP tool annotations and OpenCode v1/v2 configuration support.
- A drag-to-Applications DMG and a Sparkle 2 updater backed by an HTTPS appcast
  and Ed25519-signed release archives. Users can check from the app menu or
  Settings; Sparkle asks before enabling automatic checks.
- A no-`sudo` terminal installer that verifies the pinned DMG checksum and app
  signature before installing to `~/Applications`.

### Changed
- The desktop integration catalog is intentionally limited to Claude Code,
  Codex, OpenCode, and Cursor. The existing CLI adapter catalog remains
  available for repository compatibility.

### Privacy
- Local chat sources are never modified. Search and context include visible
  agent and subagent messages while omitting reasoning, tool payloads, metadata
  events, and credential-like lines. Attached chat text is treated as untrusted
  reference evidence.
- MCP setup adds only an `agentic-stack` entry and preserves unrelated Codex,
  Claude Code, Cursor, and OpenCode configuration.
